...
const SilkPlane = forwardRef(function SilkPlane({ uniforms, timeRef }, ref) {
  ...
  useFrame((_, delta) => {
    ...
    if (ref.current) {
      ref.current.material.uniforms.uTime.value = timeRef.current;
    }
  });
...
const Silk = ({ speed = 5, scale = 1, color = '#7B7481', noiseIntensity = 1.5, rotation = 0, lightMode = false }) => {
  ...
  // Time lives outside the uniforms so it keeps advancing even if a prop
  ...
  return (
    <Canvas dpr={[1, 2]} frameloop="always">
      <SilkPlane ref={meshRef} uniforms={uniforms} timeRef={timeRef} />
    </Canvas>
  );
};
